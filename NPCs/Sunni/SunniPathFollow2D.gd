...
onready var path_2d = get_node("/root/World/NPCsPath/SunniPath2D/PathFollow2D")
...
func _process(delta):
	if enter:
		if path_2d.get_unit_offset() < 0.0583:
			play_animation("RunRight")
		elif path_2d.get_unit_offset() >= 0.0583 and path_2d.get_unit_offset() < 0.7185:
			play_animation("RunUp")
		elif path_2d.get_unit_offset() >= 0.7185 and path_2d.get_unit_offset() < 0.835:
			play_animation("RunLeft")
			if door.get_active():
				door.play_animation()
		elif path_2d.get_unit_offset() >= 0.835 and path_2d.get_unit_offset() < 0.9224:
			play_animation("RunLeft")
		else:
			play_animation("RunUp")
